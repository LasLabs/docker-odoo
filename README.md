...
the `ODOO_VERSION` build argment to the version you would like to use. See more
in the [Build Arguments section](#build-arguments).
...
* Create an Wkhtmltox-XVFB container to use instead of internalizing.
* Add more compose examples.
* Odoo is currently running as root due to permissions issues with mounted
  addons. Odoo 8 is disabled due to this - it won't allow root to run it.
